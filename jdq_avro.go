...
func (a *JdqEventBatchEncoder) AppendRowChangedEvent(e *model.RowChangedEvent) (EncoderResult, error) {
	...
	if e.IsDelete() || e.IsInsert() || e.IsUpdate() {
		log.Debug("row change event: ", zap.Reflect("e", e))

		//log.Debug("row change event: StartTs: ",zap.Uint64("StartTs", e.StartTs))
		//log.Debug("row change event: CommitTs: ",zap.Uint64("CommitTs", e.CommitTs))
		//log.Debug("row change event: RowID: ",zap.Int64("RowID", e.RowID))
		//log.Debug("row change event: Table: ",zap.String("table", e.Table.String()))

		log.Debug("row change event: PreColumns: ",zap.Reflect("PreColumns", e.PreColumns))
		log.Debug("row change event: Columns: ",zap.Reflect("Columns", e.Columns))
		log.Debug("row change event: IndexColumns: ",zap.Reflect("IndexColumns", e.IndexColumns))
...
func (a *JdqEventBatchEncoder) columnToJdqNativeData(col *model.Column) (interface{}, error) {
	...
	case mysql.TypeVarchar, mysql.TypeString, mysql.TypeVarString,
		...
		//log.Info("var type: ",zap.Reflect("col", col))
		if col.Flag.IsBinary() {
			switch val := col.Value.(type) {
			case string:
				//log.Info("is binary type string: ",zap.Reflect("col", col))
				// return []byte(val), nil
				return val, nil
			case []byte:
				log.Info("is binary type []byte: ",zap.Reflect("col", col))
				// return ISO8859-1
